...
const Errors = require("../api/errors/subjectman-xiii-error.js");
...
const logger = LoggerFactory.get("SubjectmanXiiiAbl");

class SubjectmanXiiiAbl {
  constructor() {
  ...
  async sayHello(awid, dtoIn) {
    ...
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
      WARNINGS.sayHelloUnsupportedKeys.code, //Pro každou metodu by měl být warning
      Errors.SayHello.InvalidDtoIn
    );
    const { name } = dtoIn; //Musím to mít ve validačních typech
    return {
      uuAppErrorMap,
      msg: name,
      awid,
...
module.exports = new SubjectmanXiiiAbl();
